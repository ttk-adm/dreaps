...
mod tests {
    ...
    fn test_sum_of_products() {
        ...
        let weights: Vec<f64> = vec![0.1, 0.2, 0.3];
        let data = StatsData::new_weighted(data.x.array, data.y.array, weights, WeightMode::Statistical);
        assert_eq!(data.sum_of_products(), 150.);
        let data = StatsData::new_weighted(data.x.array, data.y.array, data.y.weights, WeightMode::Instrumental);
